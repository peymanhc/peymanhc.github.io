...
export const timeDiffrenced = (timeStart, timeEnd) => {
  const startTime = moment(timeStart, "HH:mm:ss");
  const endTime = moment(timeEnd, "HH:mm:ss");
  const duration = moment.duration(endTime.diff(startTime));
  const h = parseInt(duration.asHours());
  const m = parseInt(duration.asMinutes()) % 60;
  const s = parseInt(duration.asSeconds()) % 60;
  return { h, m, s };
};
